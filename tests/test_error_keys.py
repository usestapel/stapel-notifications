...
stapel-notifications carries the backend ``errors.json`` artifact — the
...
def test_service_keys_present_with_declared_remediation():
    ...
    # Notifications-declared canon (backend overrides the frontend heuristic:
    # a 404 not_found would otherwise heuristically resolve to `retry`).
    assert entries["error.400.invalid_platform"]["remediation"] == "fix_input"
